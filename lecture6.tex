...
\begin{theorem}[Galois correspondence]
  ...
  \item The following statement are equivalent (if and only if)
    ...
    \footnote{
      ...
      the kernel: $\exists \phi \in Hom(G) \mid \ker{\phi} =N$.
    }
  \end{enumerate}
  \begin{proof}
    \begin{enumerate}
    ...
    \footnote{
      ??? For equality $g\left(F\right) = F$ we have to proof that
      $\forall g \in Gal\left(L/K\right): F \subset g\left(F\right)$      
    }
