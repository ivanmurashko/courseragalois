...
\begin{definition}[K-algebra]
  Let $K$ be a field and $A$ be a \mynameref{def:vectorspace} over $K$
  equipped with 
...
Any \mynameref{def:homomorphism} of fields is
...
\begin{example}
  ...
  $X-a$ is
  \mynameref{def:irreducible} (see example \ref{ex:irreducible}).

  For the proof the main statement (\ref{eq:lec1_ex1_1}) lets consider a
  polynomial $P \in  K\left[X\right]$ and 
  define the following \mynameref{def:homomorphism}:
  \begin{equation}
  ...
  \mynameref{def:homomorphism}. For the proof of the claim lets take
...
\mynameref{def:field}. The $\left(P\right)$ is a \mynameref{def:maxideal}
\footnote{
...
There is a GAP \cite{url:gap} example below (not part of the lectures)
