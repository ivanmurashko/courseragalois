...
\begin{definition}[K-algebra]
  Let $K$ be a field and $A$ be a \indexref{def:vectorspace} over $K$
  equipped with 
...
\begin{definition}[Field extension]
  ...
  $L$ is an extension of $K$ if $L$ is a \index{K-algebra}
  \footnote{
...
\begin{lemma}[K-algebra and \indexref{def:homomorphism}]
Given a K-algebra is the same as
having \indexref{def:homomorphism} $f: K \rightarrow A$ of rings.
\begin{proof}
Really if I have a K-algebra I can define the
\indexref{def:homomorphism} $f(k) = k \cdot 1_A$, where $1_A$ is an
identity element of $A$. Thus $k \cdot 1_A \in A$.

And conversely if I have the \indexref{def:homomorphism}
$f: K \rightarrow A$
...
\begin{lemma}[About \indexref{def:homomorphism} of fields]
Any \indexref{def:homomorphism} of fields is
\indexref{def:injection}.
\footnote{
  But the statement is not valid for groups. In that case the
  homomorphism is injection if and only if the 
  \index{kernel} is trivial and consists of only one element - identity.
...
\indexref{def:homomorphism} can be set between field $K$ and its
K-algebra.
The \indexref{def:homomorphism} is \indexref{def:injection}
therefore we can allocate a sub-field $A' \subset A$ for that we will
have the \indexref{def:homomorphism} is a \indexref{def:surjection} and
therefore we have an \indexref{def:isomorphism} between original field $K$
...
\begin{example}[$K$-algebra is not a field]
  ...
  $K = \mathbb{R}$. \indexref{def:vectorspace} $A = \mathbb{R}^2$ i.e.
  ...
  It can be seen that all requirements of \indexref{def:kalgebra} are
  satisfied
  \begin{eqnarray}
    (x + y) \cdot z =
    \left(
...
\label{lec1_sec14}
Let $K\left[X\right]$ \indexref{def:ring} of polynomials.
The $P \in K\left[X\right]$ is an
\indexref{def:irreducible}. $\left(P\right)$ is an 
\indexref{def:ideal} formed by the polynomial.
\footnote{
  I.e.
  \(
...
Then with
\indexref{lem:bezout} we can get $\exists A, B \in K\left[X\right]$
such that
\[
A P + B Q = 1,
\]
...
\begin{example}
  ...
  \indexref{def:isomorphism} between the field and $K$ i.e.
  \begin{equation}
  ...
  $X-a$ is
  \indexref{def:irreducible} (see example \ref{ex:irreducible}).

  For the proof the main statement (\ref{eq:lec1_ex1_1}) lets consider a
  polynomial $P \in  K\left[X\right]$ and 
  define the following \indexref{def:homomorphism}:
  \begin{equation}
  ...
  \indexref{def:homomorphism}. For the proof of the claim lets take
  ...
  Next we should show that $\phi$ is \indexref{def:surjection} it's
  ...
  \indexref{thm:firstisomorphism} theorem. 
...
\indexref{def:field}. The $\left(P\right)$ is a \indexref{def:maxideal}
\footnote{
  To prove that $\left(P\right)$ is a \indexref{def:maxideal} we have
  to use \indexref{lem:bezout}.
}
but a quotient by a  \indexref{def:maxideal} is a \indexref{def:field}
(see theorem \indexref{thm:maxideal}).

$K\left[X\right]/\left(P\right)$ is an extension of $K$ because it's
\indexref{def:kalgebra}.
...
\begin{lemma}[About minimal polynomial existence]
  If $\alpha$ is \indexref{def:algebraicelement} then
  ...
  \footnote{
    see also theorem \indexref{thm:irreduciblediv}
  }
  \begin{proof}
    We know that $K\left[X\right]$ is a \indexref{def:pid} and a
    polynomial $Q\left(\alpha\right) = 0$ forms an
    \indexref{def:ideal}: $I \left\{Q \in K\left[X\right] \mid
...
\begin{definition}
  ...
  sub-ring (or \indexref{def:kalgebra}) contained $K$ and $\alpha$ denoted by $K\left[\alpha\right]$.
\end{definition}

As soon as $K\left[\alpha\right]$ is a \indexref{def:kalgebra} it is a
\indexref{def:vectorspace} over $K$ generated by
\[
...
\begin{example}[$\mathbb{C}$]
  ...
  \]
  $\mathbb{C}$ is also a \indexref{def:vectorspace} generated by $1$
...
\begin{proposition}
  ...
  \item $K\left[\alpha\right]$ is a finite dimensional
    \indexref{def:vectorspace} over $K$
  ...
  \begin{proof}
    ...
    then using lemma \indexref{def:minpolynomial} $\exists
    ...
    \alpha^{d-1}$ is a finite dimensional \indexref{def:vectorspace}.
    ...
    K\left[\alpha\right]$. This is \indexref{def:injection}.
    \footnote{
      ...
      x \cdot y  \ne x \cdot z
      \]
      i.e. \indexref{def:injection} property is satisfied.
    }
    But the $K\left[\alpha\right]$ is finite dimensional
    \indexref{def:vectorspace} and a \indexref{def:homomorphism} between
    ...
    \indexref{def:field} but $\alpha$ is not algebraic. Thus $\forall P
    \in K\left[X\right]$ $P(\alpha) \ne 0$. The we have an
    \indexref{def:injection} \indexref{def:homomorphism} $i$ :
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      using lemma \indexref{lem:minpolynomial} one can get that
...
We can consider the following example (it is not a part of the
...
\begin{theorem}[The multiplicativity formula for degrees]
  ...
  \indexref{def:finiteextension} over $K$ if and only if
  $M$ is \indexref{def:finiteextension} over $L$ and
  $L$ is \indexref{def:finiteextension} over $K$. In this case
  \[
...
\begin{theorem}[About towers]
  ...
  \begin{proof}
    ...
    \indexref{def:vectorspace} over $K\left[\alpha_1\right]$ and
    so on $K\left[\alpha_1, \dots, \alpha_d\right]$ is a finite dimensional
    \indexref{def:vectorspace} over
...
\begin{theorem}
  $K \subset L \subset M$ then $M$ \indexref{def:algebraicextension}
  over $K$ if and only if $M$ algebraic over $L$ and $L$ algebraic
  over $K$. 
  \begin{proof}
    If $\alpha \in M$ is an \indexref{def:algebraicelement} over $K$ then
...
\begin{itemize}
\item $K$ is a field, $\alpha$ is an \indexref{def:algebraicelement}
  over $K$ if it is a root of a polynomial $P \in K\left[X\right]$.
\item $L$ is an \indexref{def:algebraicextension} over $K$ if
  $\forall \alpha \in L$: $\alpha$ is an algebraic over $K$
\item $L$ is a \indexref{def:finiteextension} over $K$ if $dim_K L <
  \infty$.
...
Note: that both: \indexref{lem:gauss} and \indexref{lem:eisenstein} are
valid by replacing $\mathbb{Z}$ with an \indexref{def:ufd} $R$ and
