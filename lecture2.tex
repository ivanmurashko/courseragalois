...
an \nameref{def:irreducible} moreover the root of the $P$ is in the
field (see example 
\ref{ex:F2overP}).
...
\begin{corollary}
  ...
  does not have a root in \nameref{def:fextension1} $L$ of $K$ such that
...
\begin{corollary}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      \nameref{thm:mulformuladegrees} we have
      \[
...
\begin{remark}
  The \nameref{def:isomorphism} considered in theorem \ref{thm:lec2_1}
...
\begin{example}[$X^3-2$ over $\mathbb{Q}$]
  ...
  \begin{enumerate}
    \item $\mathbb{Q}\left(\sqrt[3]{2}\right)$
      ...
      \nameref{def:automorphism} that exchanges the roots. Lets call it
      $\tau$
      \footnote{
  ...
  \footnote{
    ...
    \begin{itemize}
      \item $\forall P \in L\left[ X \right]$ $\exists \alpha \in L$
        ...
        \nameref{def:algebraicallyclosed})
...
\begin{theorem}[About Algebraic closure]
  ...
  \begin{proof}
    ...
    Let $I \subset A$ is an \nameref{def:idealset} $P\left(X_P\right)$
    ...
    the \nameref{def:maxideal} $m$ (see proposition
