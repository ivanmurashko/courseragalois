...
\mynameref{def:monicpolynomial}. \mynameref{def:fextension1} $E$ is a stem
...
an \mynameref{def:irreducible} moreover the root of the $P$ is in the
field (see example 
\ref{ex:F2overP}).
...
\]
We can use the following \mynameref{def:isomorphism}:
...
\begin{proposition}[About stem field existence]
  ...
  (\mynameref{def:isomorphism} of K-algebras) such that $f(\alpha) =
  \alpha'$. 
  \begin{proof}
    Existence: $K\left[X\right]/\left(P\right)$ can be took as the
    stem field.

    Uniquest of the \mynameref{def:isomorphism} is easy because it is
...
\begin{remark}[About stem field]
  ...
  \item In particular: If a stem field contains 2 roots of $P$ then
    $\exists!$ \mynameref{def:automorphism} taking one root into
    another.
...
\begin{corollary}
  ...
  does not have a root in \mynameref{def:fextension1} $L$ of $K$ such that
  ...
  \begin{proof}
    ...
    The
    \mynameref{def:stemfield} $L$ for $Q$ exists and it's degree is $deg Q
...
\begin{corollary}
  ...
  \begin{proof}
    ...
    $L\left[X\right]$. Let $M$ be a \mynameref{def:stemfield} of $Q$
    over $L$.
    ...
    \footnote{
      ...
      \mynameref{thm:mulformuladegrees} we have
      \[
...
\begin{theorem}[About splitting fields]
  \begin{enumerate}
    ...
    \item If $L$ and $M$ are 2 splitting fields then
      $\exists \phi: L \cong M$ (an \mynameref{def:isomorphism}). But
      the \mynameref{def:isomorphism} is not necessary to be unique.
  \end{enumerate}
  \label{thm:lec2_1}
  \begin{proof}
    ...
    can create a \mynameref{def:stemfield} $L_1 = K\left(\alpha\right)$
    for $Q$ that will be also a \mynameref{def:stemfield} for $P$.

    Over $L_1$ we have $P = (x - \alpha) R$, where $R$ is a polynomial
    with $deg R = d - 1$. We know (by induction) that
    there exists a \mynameref{def:splittingfield} $L$ for $R$ over $L_1$
    and its degree:
    \(
    ...
    \mynameref{thm:mulformuladegrees}) is $ \le d \cdot (d-1)! = d!$.
    ...
    \footnote{
      via
      the existent \mynameref{def:isomorphism}
    ...
    %% \mynameref{def:isomorphism} $\phi$) and as such it's a splitting
    ...
    %% $L_1 = K\left(\alpha\right)$ \mynameref{def:isomorphism} $L \cong M$
    %% and therefore $K$ \mynameref{def:isomorphism} $L \cong M$.    
  \end{proof}
\end{theorem}

\begin{remark}
  The \mynameref{def:isomorphism} considered in theorem \ref{thm:lec2_1}
  is not unique. A splitting field  can 
  have many \mynameref{def:automorphism} and this is in fact the subject
  of Galois theory.
\end{remark}
...
\begin{example}[$X^3-2$ over $\mathbb{Q}$]
  ...
  find \mynameref{def:automorphism}s of the field.
  ...
  \begin{enumerate}
    \item $\mathbb{Q}\left(\sqrt[3]{2}\right)$
      ...
      \item $\mathbb{Q}\left(j\right)$ \mynameref{def:automorphism}. In
  ...
  %% is the following: how many \mynameref{def:homomorphism}s to $L =
  ...
  %% unique \mynameref{def:isomorphism}
  %% \[
...
\begin{example}[$\mathbb{C}$]
  $\mathbb{C}$ is an \mynameref{def:algebraicallyclosed}. This will be
  proved later.
\end{example}

\begin{definition}[Algebraic closure]
  An algebraic closure of $K$ is a field $L$ that is
  \mynameref{def:algebraicallyclosed} and
  \mynameref{def:algebraicextension} 
  over $K$.
  \footnote{
    ...
    \begin{itemize}
      \item $\forall P \in L\left[ X \right]$ $\exists \alpha \in L$
        ...
        \mynameref{def:algebraicallyclosed})
...
\begin{theorem}[About Algebraic closure]
  Any field $K$ has an \mynameref{def:algebraicclosure}
  \begin{proof}
    ...
    Let $I \subset A$ is an \mynameref{def:idealset} $P\left(X_P\right)$
    $\forall P \in S$.\footnote
    {
      $I = \sum_i \lambda_i P_i\left(X_{P_i}\right)$, where $\lambda_i \in A$
    }
    We claim that $I$ is a \mynameref{def:properideal} i.e. $I \ne
    ...
    \mynameref{def:splittingfield} $L$ for the polynomial $P$ over $K$
    ...
    Fact: Any \mynameref{def:properideal} $I \subset A$ is contained in
    the \mynameref{def:maxideal} $m$ (see proposition
...
\mynameref{def:properideal} is in a \mynameref{def:maxideal}. This is a
...
\begin{lemma}[Zorn]
  If any non-empty \mynameref{def:chain} $\mathcal{C}$ in a non-empty set
...
\begin{proposition}
  Any \mynameref{def:properideal} is in a \mynameref{def:maxideal}
  \begin{proof}
    We can use \mynameref{lem:zorn} to prove that any proper ideal is
    in a \mynameref{def:maxideal}.
    ...
    \mynameref{def:chain} $\mathcal{C} = \left\{I_\alpha\right\}$
    \footnote{
...
If we take a \mynameref{def:quotientring} by maximal ideal it's always a
field
\footnote{
...
field $K_i$ is \mynameref{def:quotientring} of huge polynomial ring
$K_{i-1}\left[X\right]$ by a suitable \mynameref{def:maxideal} that is
got by means of \mynameref{lem:zorn}.
...
\begin{theorem}[About extension of homomorphism]
  Let $K \subset L \subset M$ - \mynameref{def:algebraicextension}.
  $K \subset \Omega$, where $\Omega$ - \mynameref{def:algebraicclosure}
  of $K$.
  ...
  \begin{proof}
    Apply \mynameref{lem:zorn} to the following set (of pairs)
    \[
    \mathcal{E} = \left\{
    ...
    Any \mynameref{def:chain} $\left(N_\alpha, \psi_\alpha\right)$
    ...
    \mynameref{def:minpolynomial} $P_{min}\left(x, N_0\right)$. It
...
\begin{corollary}[About algebraic closure isomorphism]
  ...
  \begin{proof}
    ...
    (i.e. \mynameref{def:injection}) from $\Delta'$ to $\Delta$.

    If we assume $M = \Delta$ and $\Omega = \Delta$ then there exists
    a homomorphism (i.e. \mynameref{def:injection}) from $\Delta$ to
    $\Delta'$. The \mynameref{def:injection} is also
    \mynameref{def:surjection} in another direction: $\Delta' \to
    \Delta$ and as result we have \mynameref{def:isomorphism} $\Delta' \to
    \Delta$ 
  \end{proof}
