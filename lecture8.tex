...
\begin{theorem}[Normal base]
  ...
  \begin{proof}
    ...
    determinant.
    \footnote{
      ??? We consider the following polynomial for each $g_i$:
      $P_i = \sum_{i=1}^n k_{ij} X_j$, where
      $P_i \in K\left[X_1, \dots, X_n\right]$ and 
      $k_{ij} \in K$.  
    }
    ...
    \footnote{
      ...
      $P \in K\left[X_1, \dots, X_n\right]$ can vanish at every point
    ...
    \]
    as a $K\left[X\right]$-module with $X$ acting by $\sigma$. This is a cyclic
    module
    \footnote{
      ??? $ = \left<x\right>$
    }
