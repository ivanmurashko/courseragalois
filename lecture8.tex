...
\begin{claim}
  ...
  \nameref{def:galoisextension}s over $K$ then
  \[
  ...
  \begin{proof}
    ...
    \]
    The \nameref{thm:galoiscorrespondence} theorem says that $\phi$ is
    a \nameref{def:surjection} with kernel
    $Gal\left(L/F\right)$. Therefore by
    \nameref{thm:firstisomorphism} theorem one can get the required
    statement. 
  \end{proof}
...
a \nameref{def:quotientofgroupaction}. The covering is characterized
by the property that $Y 
\times_X Y = \sqcup_{g \in G} Y_g$ (is a \nameref{def:disjointunion}),
$Y_g = \{\left(y, gy\right)\}$. 
\footnote{
  ??? add an explanation
}
