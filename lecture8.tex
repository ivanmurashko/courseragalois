...
into its \indexref{def:algebraicclosure}.
...
\begin{definition}[Polynomial solvable by radicals]
  ...
  $E$ - \indexref{def:solvableextension} and containing all roots of
  $P$. 
...
\begin{property}
  \begin{enumerate}
  \item \indexref{def:compositeextension} of solvable by radicals is itself
    solvable by radicals
  \item If $L$ extension of $K$ is solvable by radicals (by definition
    $L$ should be finite extension of $K$) then exists a finite
    \indexref{def:galoisextension} $E$ containing $L$ and solvable by
    radicals.    
  \end{enumerate}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      By \indexref{thm:galoiscorrespondence} (item
      ...
      \indexref{thm:galoiscorrespondence} (item
...
\begin{definition}[Solvable group]
  ...
  \indexref{def:normalsubgroup} of 
  $G_{i-1}$ and the \indexref{def:quotientgroup} $G_{i-1}/G_i$ is
  an \indexref{def:abeliangroup}.
...
\begin{example}[Group of permutations $S_4$]
  ...
  \right\}.
  \]
  A double transposition is a product of two \indexref{def:transposition}s with
  ...
  $K$ is \indexref{def:abeliangroup} and
...
\begin{property}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      \indexref{def:abeliangroup} and now we have to prove that the
      \indexref{def:quotientgroup} 
      ...
      \indexref{def:homomorphism}. Thus $\forall a,b \in H_{i-1}/H_i$
      ...
      \indexref{def:abeliangroup} and as result $H$ is solvable.
    }
  \end{proof}
...
\begin{property}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      and $\bar{G}_{i-1}/\bar{G}_i$ is \indexref{def:abeliangroup}.
      ...
      ${G}_{i-1}/{G}_i$ is \indexref{def:abeliangroup} therefore
      $\forall a,b \in {G}_{i-1}: a b G_i = b a G_i$. Using the fact
      that $H$ is a \indexref{def:normalsubgroup} one can get
      \begin{eqnarray}
        ...
        \nonumber 
      \end{eqnarray}
      i.e. $\bar{G}_{i-1}/\bar{G}_i$ is \indexref{def:abeliangroup}.
    }
  \end{proof}
...
\begin{property}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      \nonumber
      \end{eqnarray}
      i.e. the \indexref{def:quotientgroup} $G_{i-1}/G_i$ is an Abelian
      group.
...
\begin{property}
  If $G$ is finite \indexref{def:abeliangroup} than $G$ is solvable and
  there exists a finite filtration with cyclic quotients for $G$.
  \begin{proof}
    Accordingly theorem \ref{thm:fgagroup}) each finite
    \indexref{def:abeliangroup} $G$ can be represented as
    \[
    G = K_1 \oplus K_2 \oplus \dots \oplus K_{n-1} \oplus K_n,
    \]
    where $K_i$ is a \indexref{def:cyclicgroup}. If we denote
    \[
...
\begin{property}
  ...
  \begin{proof}
    This is just because a finite \indexref{def:abeliangroup} is just a
    product of cyclic groups (see see \indexref{thm:fgagroup}).
    \footnote{
       If exists a finite filtration of $G$ with cyclic quotient then
      the $G$ is solvable because \indexref{def:cyclicgroup} is
      ...
      follows from \indexref{thm:fgagroup}) says us that there exists a
      ...
      K_0 = K
      \]
      where $K^{(j-1)}/K^{(j)}$ is cyclic. The \indexref{thm:correspondence}
      ...
      %% The $K$ is a finite \indexref{def:abeliangroup}. But by
      %% theorem \ref{thm:simple_subgroup_of_abelian} any finite abelian
      %% group $K$ has a simple (see \indexref{def:simplegroup}) subgroup
...
\begin{definition}[Solvable group]
  ...
  \indexref{def:commutatorsubgroup}.
...
\begin{remark}
  ...
  \begin{proof}
    Our filtration with \indexref{def:commutatorsubgroup}s
  ...
  \begin{proof}
    ...
    \begin{enumerate}
      \item we know that $\left[S_n, S_n\right] = A_n$ - subgroup of
        ...
        any 3-cycle is a \indexref{def:commutatorsubgroup}
        \footnote{
      ...
      \item If $n \ge 5$ then $\left[A_n, A_n\right] = A_n$ thus the
        ...
        that all double transpositions are in the $\left[A_n,
          A_n\right]$ (\indexref{def:commutatorsubgroup} of $A_n$).
        \footnote{
...
\begin{claim}
  ...
  \indexref{def:galoisextension}s over $K$ then
  \[
  \frac{Gal\left(M/K\right)}{Gal\left(M/L\right)}
  \cong Gal\left(L/K\right).
  \]
  \begin{proof}
    Consider the following group \indexref{def:homomorphism}
    \[
    \phi:Gal\left(M/K\right) \xrightarrow[g \to g\mid_L]{} Gal\left(L/K\right).
    \]
    The \indexref{thm:galoiscorrespondence} theorem says that $\phi$ is
    a \indexref{def:surjection} with kernel
    $Gal\left(L/F\right)$. Therefore by
    \indexref{thm:firstisomorphism} theorem one can get the required
    statement. 
  \end{proof}
...
\begin{theorem}
  ...
  \indexref{def:solvablepolynomial} if and only if
  $Gal\left(P\right)$ is solvable. There
  $Gal\left(P\right)$ is (by definition) $Gal\left(F/K\right)$ where
  $F$ is a \indexref{def:splittingfield} of $P$ over $K$.
  \label{thm:lec8_1}
  \begin{proof}
    ...
    \footnote{
      $g(F) = F$ see \indexref{thm:galoiscorrespondence} (point
    ...
    \footnote{
      ...
      \indexref{thm:lagrange} theorem one can get that
    ...
    \footnote{
      ...
      M_r = M^{G_r} = M^{\{e\}} = M.
      \]
      Thus by \indexref{thm:galoiscorrespondence},
      as soon as
      ...
      \indexref{def:galoisextension} over $M_i$ with
      \indexref{def:galoisgroup} $G_i/G_{i+1}$
...
\begin{example}[Not solvable polynomial of degree 5]
  ...
  \begin{enumerate}
    \item $Gal\left(P\right)$ contains the complex conjugation (we
      have 2 complex conjugated roots but \indexref{def:galoisgroup} is
    ...
    \item As soon as $P$ is irreducible then $Gal\left(P\right)$
      ...
      (see \indexref{thm:orbitstabilizertheorem})
      $\left|G\right| = \left|Orb\left(x\right)\right| \left|G_x\right|$
      ($Orb\left(x\right)$ - is the \indexref{def:orbit},
      ...
      the cardinality of $G$. This means, by \indexref{cor:sylow} 
...
\begin{definition}[Group representation]
  Let $G$ is a finite group. $V$ is a \indexref{def:vectorspace} over
  $K$. Representation of $G$ is a \indexref{def:homomorphism}
  $\rho: G \to GL\left(V\right)$
  (where $GL\left(V\right)$ is the \indexref{def:glv} i.e. the group of
  \indexref{def:automorphism}s of the vector space $V$).
...
\indexref{def:galoisgroup} $Gal\left(L/K\right)$:
...
\indexref{def:regularrepresentation}.
\begin{definition}[Regular representation]
  ...
  permutations:
  \footnote{
    $\rho_{reg}\left(h\right)$ is an \indexref{def:automorphism} of $V$
    i.e.
    \[
...
The $\bar{K}^n$ is a \indexref{def:directsum} of $n$
...
be obtained by group \indexref{def:action} $j \circ g, g \in G$.
\footnote{
  This is because the \indexref{def:galoisgroup} acts transitively on
  the set of homomorphisms from $L$ to $\bar{K}$ ($Hom_K\left(L,
  \bar{K}\right)$), see theorem \ref{thm:lec5_3}. 
}
So
$\bar{K}^n$ has a basis indexed by $G$ and the \indexref{def:action} of
$G$ permutes the basis vectors.
\footnote{
  We have $L$ is a \indexref{def:galoisextension} i.e. (via
  \indexref{thm:primitiveelement} theorem) $\exists \alpha \in L$
...
\bar{K}^n \cong R$, where R is a \indexref{def:regularrepresentation}
of $G$ over $\bar{K}$. 
...
\begin{theorem}[Normal base]
  ...
  $G$ are
  linearly independent in the space of \indexref{def:endomorphism}s
  $End_{K}\left(L\right)$
  ...
  \begin{proof}
    ...
    not apply any more. But in the case the \indexref{def:galoisgroup} is
    ...
    \indexref{def:endomorphism} of $L$ over $K$ is $X^n - 1$.
...
\begin{remark}
  If $L$ is a finite \indexref{def:galoisextension} of $K$ then
...
a \indexref{def:quotientofgroupaction}. The covering is characterized
by the property that $Y 
\times_X Y = \sqcup_{g \in G} Y_g$ (is a \indexref{def:disjointunion}),
$Y_g = \{\left(y, gy\right)\}$. 
\footnote{
  ??? add an explanation
}
