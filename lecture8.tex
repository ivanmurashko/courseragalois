...
\begin{theorem}[Normal base]
  ...
  \begin{proof}
    ...
    \footnote{
      ??? $K$-basis of $K\left[X\right]/\left(X^n - 1\right)$ is the
      following $1, X, \dots, X^{n-1}$
      \cite{bib:KeithConradLinearchar}. Each element of the basis is a 
      linear map $L \to L$. I.e. the image of the basis will also be
      $K$-basis of $L$ (as soon as $L \cong K\left[X\right]/\left(X^n
      - 1\right)$). I.e. if $x \in L$ is a generator of $L$ as
      a $K\left[X\right]$-module then 
