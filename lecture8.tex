...
\footnote{
  ...
  If
  \[
  e_1 = (\alpha, \alpha^2, \dots, \alpha^{n-1}) =
  (\phi_1(\alpha), \phi_1(\alpha^2), \dots, \phi_1(\alpha^{n-1}))
  \]
  is a basis vector then $g_i: \alpha \to \alpha_i$ gives us another one:
  \begin{eqnarray}
  g_i e_1 =
  (\phi_1(g_i \alpha), \phi_1(g_i \alpha^2), \dots, \phi_1(g_i
  \alpha^{n-1})) =
  \nonumber \\
  =(\phi_1(\alpha_i), \phi_1(\alpha_i^2), \dots, \phi_1(
  \alpha_i^{n-1})) =  
  \nonumber \\
  = (\phi_i(\alpha), \phi_i(\alpha^2), \dots, \phi_i(\alpha^{n-1})) = e_i
  \nonumber
  \end{eqnarray}
}
...
\footnote{
  We have an isomorphism $L \otimes_K \bar{K} \cong R$, where $R$ is
  \nameref{def:regularrepresentation}. If $\{e_g\}$ is the basis of
  $R$ we can pick up one element of the basis, for instance $e_{id}$
  and find the corresponded element in $L \otimes_K \bar{K}$,
  $x \in L \otimes_K \bar{K}$,
  such that $\phi(x) = e_{id}$, where $\phi$ is the isomorphism. If $g
  \in G$ then ??? $\phi(g(x)) = g\left(\phi(x)\right) = g(e_{id}) =
  e_g$. I.e. $\{g(x)\mid g \in G\}$ is the basis of $L \otimes_K \bar{K}$.
...
\begin{theorem}[Normal base]
  ...
  \begin{proof}
    ...
    \footnote{
      ??? Let there is no $l \in L$ such that $g(l)$ is the basis of $L$
      (i.e. determinant is 0).
      But there exists $x = l \otimes k \in L \otimes_{\bar{K}} \bar{K}$
      such that $g(x)$ forms a basis i.e.
      $\{g(l) \otimes k\mid g \in G\}$ is $\bar{K}$ basis of $L
      \otimes_{\bar{K}} \bar{K}$, but in the case $\{g(l) \otimes
      1_K\mid g \in G\}$ will also be a $\bar{K}$ basis. Therefore
      accordingly proposition \ref{prop:lec4_Addon},
      $\{g(l)\mid g \in G\}$ forms a $K$-basis in $L$ that is in a
