...
\begin{example}[Group of permutations $S_4$]
  ...
  is a subgroup
  \footnote{
    There is a well known \index{Klein four group}
...
be obtained by group \mynameref{def:action} $j \circ g, g \in G$.
\footnote{
  This is because the \mynameref{def:galoisgroup} acts transitively on
  the set of homomorphisms from $L$ to $\bar{K}$ ($Hom_K\left(L,
  \bar{K}\right)$), see theorem \ref{thm:lec5_3}. 
}
So
$\bar{K}^n$ has a basis indexed by $G$ and the \mynameref{def:action} of
$G$ permutes the basis vectors.
\footnote{
  We have $L$ is a \mynameref{def:galoisextension} i.e., via
  \mynameref{thm:primitiveelement}, $\exists \alpha \in L$
...
\bar{K}^n \cong R$, where R is a \mynameref{def:regularrepresentation}
of $G$ over $\bar{K}$. 
...
\begin{theorem}[Normal base]
  ...
  $G$ are
  linearly independent in the space of \mynameref{def:endomorphism}s
  $End_{K}\left(L\right)$
  ...
  \begin{proof}
    ...
    not apply any more. But in the case the \mynameref{def:galoisgroup} is
    ...
    \mynameref{def:endomorphism} of $L$ over $K$ is $X^n - 1$.
...
\begin{remark}
  If $L$ is a finite \mynameref{def:galoisextension} of $K$ then
...
a \mynameref{def:quotientofgroupaction}. The covering is characterized
by the property that $Y 
\times_X Y = \sqcup_{g \in G} Y_g$ (is a \mynameref{def:disjointunion}),
$Y_g = \{\left(y, gy\right)\}$. 
\footnote{
  ??? add an explanation
}
