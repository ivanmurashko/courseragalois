...
\footnote{
  It is because there are $\left[L:K\right]$ roots of a
  polynomial and all the roots are in $\bar{K}$. Each root $\alpha_i$
  forms a polynomial $X - \alpha_i$ which creates an ideal $m_i =
  \left(X - \alpha_i\right)$. See also comments to theorem
  \ref{thm:lec5_1} proof below (\ref{footnote:lec5_productexpl}). They  
  will describe the situation more precisely. 
}
In the case
\[
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \item $L$ separable is the same thing that the algebra
      ...
      \footnote{
        \label{footnote:lec5_productexpl}
