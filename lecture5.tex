...
\footnote{
  ??? As it was mentioned on \cite{wiki:localring}, a nonzero ring in
  which every element is either a unit or nilpotent is a local
  ring. But not reverse as it was mentioned on the lectures. There is
  also example $A \cong A/\{0\}$ where $A$ is a \nameref{def:field}
  and $\{0\}$ is the only maximal ideal for the fields. In this case
  there are many non nilpotents different from identity and zero.
}
.
...
\begin{corollary}[Primitive element theorem]
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      have to be \nameref{def:algebraicelement}s over $K$ and $\beta$
      has to be \nameref{def:degsepelem}. Let $\lambda \in K$ and we
