...
\]
This is a general form of \mynameref{thm:chineseremainder}.
...
\begin{example}
  ...
  Then by the  \mynameref{thm:chineseremainder}
  \footnote{
  ...
  \footnote{
    ...
    we have $\left(P_i\right)$ is a \mynameref{def:maxideal} for
    $K\left[X\right]$ and 
    \[
...
\begin{definition}[Nilpotent element]
  Let $A$ is a \mynameref{def:ring} than $x \in A$ is nilpotent if $x \ne 0$ but
  $\exists k: x^k = 0$.
  \footnote{
...
\begin{definition}[reduced]
  $K$-algebra $A$ is reduced if it has no \mynameref{def:nilpotent}s.
  Or in other words
  \footnote{
  ...
  Or 
  \footnote{
    $A/m_i$ is a field as soon as $m_i$ is a \mynameref{def:maxideal}
  }
...
\begin{definition}[local]
  \mynameref{def:ring} $A$ is called local if it has only one
  \mynameref{def:maxideal} i.e. $A \cong A/m^k$. 
  \label{def:local}
\end{definition}
...
\footnote{
  ...
  also an example $A \cong A/\{0\}$ where $A$ is a \mynameref{def:field}
...
\footnote{
  ...
  \ref{ex:lec3_notperfect}) it's not a  \mynameref{def:perfectfield} and
...
then the number of \mynameref{def:homomorphism}s
...
Lets also recall \mynameref{thm:basechange}. If $L$ and $E$ are
...
\begin{definition}
  With \mynameref{thm:chineseremainder} we have
  \[
  A \cong A/m_1^{k_1} \times \dots \times A/m_r^{k_r}
  \]
...
\]
where $\eta\left(A\right)$ is an \mynameref{def:ideal} of
nilpotents in $A$.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \item $L$ separable is the same thing that the algebra
      ...
      $\bar{K}$ which is the same as $A$ is \mynameref{def:reduced}
      ...
      as result $A$ is \mynameref{def:local}.
    \item If $\Omega$ is an algebraic extension then
      \footnote {
        i.e. we can setup an \mynameref{def:injection} and structure
...
\mynameref{def:vectorspace}s. So, for my field extensions, I can say
...
\begin{theorem}[Primitive element]
  Let $L$ is a finite \mynameref{def:separableextension} of $K$ then it
  ...
  \begin{proof}
    ...
    \mynameref{def:idempotent}s namely by
    \[
    ...
    finitely many \mynameref{def:idempotent}s because 
...
\begin{corollary}[Primitive element theorem]
  ...
  \begin{proof}
    ...
    If $L$ and $K$ are \mynameref{def:finitefield}s, then we have
...
\begin{definition}[Normal extension]
  A normal extension of $K$ is a \mynameref{def:splittingfield} of a
...
\begin{remark}[Normal extension]
  ...
  For instance, a \mynameref{def:splittingfield} of one polynomial is
...
\begin{theorem}
  ...
  \item $L$ is \mynameref{def:normalextension}
  \item All \mynameref{def:homomorphism}s from $L$ to $\bar{K}$ have the
    same image.
    \footnote{
      And the image is $L$ as soon as $id$ is also a
      \mynameref{def:homomorphism}
      ...
      \item You can't naturally identify $L$ with a particular
        ...
        2.5 (see theorem \mynameref{thm:lec2_3}) we know, that there
      ...
      \item My previous answer was a little bit misleading. I meant,
        ...
        %% is $\mathbb{Q}(\sqrt[3]{2}, e^{2\pi i/3})$. So, yes,
  ...
  \item The \mynameref{def:group} of \mynameref{def:automorphism}s
  ...
  \begin{proof}
    ...
    4 implies 1:  I have this \mynameref{def:transitive} and I have to prove
...
\begin{definition}[Galois extension]
  A Galois extension is a \mynameref{def:normalextension} and
  \mynameref{def:separableextension}.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    whenever (if and only if) \mynameref{def:action} is
    \mynameref{def:transitive}.
    \footnote{
      This follows directly from the definition of
      \mynameref{def:transitive} i.e. $\forall h_1, h_2 \in
    ...
    it's \mynameref{def:galoisextension}.
  \end{proof}
\end{theorem}
...
\begin{remark}[on normal extensions]
  ...
  \begin{enumerate}
  \item If we have an \mynameref{def:isomorphism} of sub-extensions
    ($K \subset L_1, L_2 \subset L$)
    $\phi: L_1 \cong L_2$ then it extends to an
    \mynameref{def:automorphism} of $L$.  To see this, we embed $L$ into
  ...
  \item \label{rem:item:lec5_onnormalext_2} The group of automorphisms $Aut\left(L/K\right)$ acts
    ...
    \footnote{
      In the case we have \mynameref{def:transitive} i.e. for any roots
  ...
  \item If the group $Aut\left(L/K\right)$ fixes (see definition
    ...
    \mynameref{def:galoisextension} (i.e. is separable) then
...
\begin{definition}[Galois group]
  If $L$ is \mynameref{def:galoisextension} then Galois group $G =
...
\begin{theorem}[Artin]
  ...
  \item If $G$ acts with finite orbits,  so, I mean all orbits of G
    ...
    then $L$ is a \mynameref{def:galoisextension} of $L^G$. 
  \item If $\left|G\right| = n < \infty$ then
    $\left[L : L^G\right] = n$ and $G$ is a \mynameref{def:galoisgroup}
  \end{enumerate}
  ...
  \begin{proof}
    ...
    \item  Let me take $x$, well say, $x_1 \in L$ which is not
      ...
      \mynameref{def:galoisextension} over $L^G$ by the
      \mynameref{def:galoisextension} definition.
    \item We have $\left|G\right| = n$ then
      ...
      direct consequence of \mynameref{thm:primitiveelement}. I
      ...
      and I can apply
      \mynameref{thm:primitiveelement}. Therefore
