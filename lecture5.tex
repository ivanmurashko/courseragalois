...
\]
This is a general form of \indexref{thm:chineseremainder} theorem.
...
\begin{example}
  ...
  Then by the  \indexref{thm:chineseremainder} theorem
  \footnote{
  ...
  \footnote{
    ...
    we have $\left(P_i\right)$ is a \indexref{def:maxideal} for
    $K\left[X\right]$ and 
    \[
...
\begin{definition}[Nilpotent element]
  Let $A$ is a \indexref{def:ring} than $x \in A$ is nilpotent if $x \ne 0$ but
  $\exists k: x^k = 0$.
  \footnote{
...
\begin{definition}[reduced]
  $K$-algebra $A$ is reduced if it has no \indexref{def:nilpotent}s.
  Or in other words
  \footnote{
  ...
  Or 
  \footnote{
    $A/m_i$ is a field as soon as $m_i$ is a \indexref{def:maxideal}
  }
...
\begin{definition}[local]
  \indexref{def:ring} $A$ is called local if it has only one
  \indexref{def:maxideal} i.e. $A \cong A/m^k$. 
  \label{def:local}
\end{definition}
...
\footnote{
  ...
  also an example $A \cong A/\{0\}$ where $A$ is a \indexref{def:field}
...
\footnote{
  ...
  \ref{ex:lec3_notperfect}) it's not a  \indexref{def:perfectfield} and
...
then the number of \indexref{def:homomorphism}s
...
Lets also recall \indexref{thm:basechange}. If $L$ and $E$ are
...
\begin{definition}
  With \indexref{thm:chineseremainder} theorem we have
  \[
  A \cong A/m_1^{k_1} \times \dots \times A/m_r^{k_r}
  \]
...
\]
where $\eta\left(A\right)$ is an \indexref{def:ideal} of
nilpotents in $A$.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \item $L$ separable is the same thing that the algebra
      ...
      $\bar{K}$ which is the same as $A$ is \indexref{def:reduced}
      ...
      as result $A$ is \indexref{def:local}.
    \item If $\Omega$ is an algebraic extension then
      \footnote {
        i.e. we can setup an \indexref{def:injection} and structure
...
\indexref{def:vectorspace}s. So, for my field extensions, I can say
...
\begin{theorem}[Primitive element]
  Let $L$ is a finite \indexref{def:separableextension} of $K$ then it
  ...
  \begin{proof}
    ...
    \indexref{def:idempotent}s namely by
    \[
    ...
    finitely many \indexref{def:idempotent}s because 
...
\begin{corollary}[Primitive element theorem]
  ...
  \begin{proof}
    ...
    If $L$ and $K$ are \indexref{def:finitefield}s, then we have
...
\begin{definition}[Normal extension]
  A normal extension of $K$ is a \indexref{def:splittingfield} of a
...
\begin{remark}[Normal extension]
  ...
  For instance, a \indexref{def:splittingfield} of one polynomial is
...
\begin{theorem}
  ...
  \item $L$ is \indexref{def:normalextension}
  \item All \indexref{def:homomorphism}s from $L$ to $\bar{K}$ have the
    same image.
    \footnote{
      And the image is $L$ because $id$ is also a
      \indexref{def:homomorphism}
  ...
  \item The \indexref{def:group} of \indexref{def:automorphism}s
  ...
  \begin{proof}
    ...
    4 implies 1:  I have this \indexref{def:transitive} and I have to prove
...
\begin{definition}[Galois extension]
  A Galois extension is a \indexref{def:normalextension} and
  \indexref{def:separableextension}.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    whenever (if and only if) \indexref{def:action} is
    \indexref{def:transitive}.
    \footnote{
      This follows directly from the definition of
      \indexref{def:transitive} i.e. $\forall h_1, h_2 \in
    ...
    it's \indexref{def:galoisextension}.
  \end{proof}
\end{theorem}
...
\begin{remark}[on normal extensions]
  ...
  \begin{enumerate}
  \item If we have an \indexref{def:isomorphism} of sub-extensions
    ($K \subset L_1, L_2 \subset L$)
    $\phi: L_1 \cong L_2$ then it extends to an
    \indexref{def:automorphism} of $L$.  To see this, we embed $L$ into
  ...
  \item The group of automorphisms $Aut\left(L/K\right)$ acts
    ...
    \footnote{
      In the case we have \indexref{def:transitive} i.e. for any roots
  ...
  \item If the group $Aut\left(L/K\right)$ fixes (see definition
    ...
    \indexref{def:galoisextension} (i.e. is separable) then
...
\begin{definition}[Galois group]
  If $L$ is \indexref{def:galoisextension} then Galois group $G =
...
\begin{theorem}[Artin]
  ...
  \item If $G$ acts with finite orbits,  so, I mean all orbits of G
    ...
    then $L$ is a \indexref{def:galoisextension} of $L^G$. 
  \item If $\left|G\right| = n < \infty$ then
    $\left[L : L^G\right] = n$ and $G$ is a \indexref{def:galoisgroup}
  \end{enumerate}
  ...
  \begin{proof}
    ...
    \item  Let me take $x$, well say, $x_1 \in L$ which is not
      ...
      \indexref{def:galoisextension} over $L^G$ by the
      \indexref{def:galoisextension} definition.
    \item We have $\left|G\right| = n$ then
      ...
      direct consequence of \indexref{thm:primitiveelement} theorem. I
      ...
      and I can apply
      \indexref{thm:primitiveelement} theorem. Therefore
