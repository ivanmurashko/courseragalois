...
\begin{definition}[Quotient of the group action]
  Consider \cite{wiki:groupaction} a group $G$ acting on a set
  $X$. The set of all \nameref{def:orbit}s of $X$ under the action of
...
\begin{property}[Quotient of direct product]
  ...
  \begin{proof}
    ...
    Therefore by \nameref{thm:firstisomorphism} theorem one can get
    \[
    G/\ker{\pi} \cong G_2,
    \]
...
\begin{example}[Transposition product]
  ...
  The example shows a product of 2 \nameref{def:transposition}s with a
  same element:
  \begin{eqnarray}
    (a,c)(a,b) = 
    \begin{array}{c}
...
\begin{corollary}
  $S_n$ is generated by any combination of a \nameref{def:transposition} and
...
\begin{example}[Multiplicative group of integers modulo $n$]
  ...
  \nameref{def:propersubgroup} not equal to $id$.
...
\begin{lemma}[$n\mathbb{Z}$ prime ideal]
  ...
  \nameref{def:primeideal} in $\mathbb{Z}$ \cite{wiki:primeideal}
  \label{lem:primeideal_in_Z}
\end{lemma}

\begin{lemma}
  All nonzero \nameref{def:primeideal}s are maximal in a principal
...
\begin{definition}[Cyclic module]
  A (left) $R$-\nameref{def:module} $M$ is called cyclic if $M$ can be
  generated by 
...
\begin{definition}[Dual basis]
  ...
  \nameref{def:dualspace} such that $v_j(e_i) = \delta_{ij}$ and is
