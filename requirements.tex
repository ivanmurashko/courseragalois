...
\begin{definition}[Group]
  ...
  \label{def:group}
\end{definition}
Therefore the group is a \indexref{def:monoid} with inverse element
property. 
...
\begin{definition}[Subgroup]
  Let we have a \indexref{def:group} $\left(G, \circ\right)$. The
  subset $S \subset G$ is called as subgroup if $\left(S,
  \circ\right)$ is a \indexref{def:group}.
  \label{def:subgroup}
\end{definition}

\begin{definition}[Proper subgroup]
  A proper subgroup of a group $G$ is a \indexref{def:subgroup} $H$
...
\begin{definition}[Normal subgroup]
  ...
  We also can write normality in the \indexref{def:coset} notation as
  follows.
...
\begin{definition}[Simple group]
   ...
   \indexref{def:normalsubgroup}s are the trivial group and the group
   itself.
...
\begin{definition}[Quotient group]
  ...
  \indexref{def:cyclicgroup} of addition modulo n can be obtained from
  the integers by 
  ...
  left \indexref{def:coset}s (they also equal to the right cosets as
  soon as $N$ is a \indexref{def:normalsubgroup}):
  \[
  G/N = \left\{a N: a \in G\right\}
  \]

  See also example \indexref{ex:s3a3quotientgroup}.
...
\begin{example}[Quotient group]
  ...
  subgroup, because $\mathbb{Z}$ is \indexref{def:abeliangroup}. There
  are only two \indexref{def:coset}s: the set 
  of even integers and the set of odd integers; therefore, the
  quotient group $\mathbb{Z}/2\mathbb{Z}$ is the
  \indexref{def:cyclicgroup} with two 
  elements. This 
...
\begin{theorem}[Correspondence theorem]
  ...
  $N$ is a \indexref{def:normalsubgroup} of a group  $G$, then there
...
\begin{definition}[Commutator subgroup]
  ...
  subgroup generated by all the \indexref{def:commutator}s of the group 
  \cite{wiki:commutatorsubgroup}.
  \label{def:commutatorsubgroup}
\end{definition}

\begin{theorem}[About quotient group and commutator subgroup]
  Given a group $G$ a \indexref{def:quotientgroup} $G/N$ is an
  \indexref{def:abeliangroup} if and only if $N \supseteq \left[G,
    G\right]$
  \cite{wiki:commutatorsubgroup}
...
\begin{definition}[Cyclic group]
  ...
  single element.
  Note that \indexref{ex:groupZ2} is a cyclic group.
  See example \ref{ex:multiplicativegroup}.
  \label{def:cyclicgroup}
\end{definition}

\begin{theorem}[Fundamental theorem of cyclic groups]
  In abstract algebra, every subgroup of a \indexref{def:cyclicgroup} is
  cyclic. Moreover, for a finite cyclic group of order $n$, every
  \indexref{def:subgroup}'s order is a divisor of $n$, and there is
...
\begin{theorem}[Orbit-stabilizer theorem]
  ...
  \]
  where $x \in X$, $G\left(x\right)$ - is the \indexref{def:orbit},
  $G_x$ - \indexref{def:stabilizersubgroup}.
  
  Note: the result was got from \cite{wiki:groupaction} as
  orbit-stabilizer theorem + \indexref{thm:lagrange} theorem
...
\begin{definition}[Quotient of the group action]
  Consider \cite{wiki:groupaction} a group $G$ acting on a set
  $X$. The set of all \indexref{def:orbit}s of $X$ under the action of
...
\begin{definition}[Direct product]
  ...
  The direct product of 2 \indexref{def:abeliangroup}s is also called
  \indexref{def:directsum} 
  
  \label{def:directproduct}
\end{definition}

\begin{property}[Direct product of groups]
  Let $G_1$, $G_2$ - \indexref{def:group}s and $G = G_1 \times G_2$ -
  \indexref{def:directproduct} of the groups. Then
  \begin{enumerate}
  \item $G_1 \cong \left(G_1, 1_{G_2}\right)$ and
    $G_2 \cong \left(1_{G_1}, G_2\right)$
  \item $G_1$ and $G_2$ are \indexref{def:normalsubgroup}s in $G$
  \end{enumerate}
  See \cite{wiki:directproduct}.
...
\begin{property}[Quotient of direct product]
  Let $G_1$, $G_2$ - \indexref{def:group}s and $G = G_1 \times G_2$ -
  \indexref{def:directproduct} of the groups.
  Then
  \[
  G/G_1 \cong G_2
  \]
  ...
  \begin{proof}
    ...
    $\pi$ is \indexref{def:homomorphism}. Really
    ...
    \begin{eqnarray}
      ...
      \nonumber
    \end{eqnarray}
    As result $\pi$ is a \indexref{def:homomorphism}.
    
    The $\pi$ is also a \indexref{def:surjection} because $\forall a_2
    \in G_2$ $\exists a \in G$ such that $\pi(a) = a_2$. Really we can
    use $a = (1_{G_1}, a_2)$.
    
    Therefore by \indexref{thm:firstisomorphism} theorem one can get
    \[
    G/\ker{\pi} \cong G_2,
    \]
...
\begin{definition}[Abelian group]
  Let we have a \indexref{def:group} $\left(G, \circ\right)$.
...
\begin{theorem}[About order of element of an Abelian group]
  If $G$ is a finite \indexref{def:abeliangroup} and $m$ is the maximal
...
\begin{theorem}
  Let $G$ is an \indexref{def:abeliangroup} and $n = \left|G\right|$
  ...
  \begin{proof}
    ...
    \indexref{thm:lagrange} $m \mid n$ i. e. $n = k_1 m$ where $k_1 \in
...
\begin{theorem}[Cyclic Group is Abelian]
  Let $G$ be a \indexref{def:cyclicgroup} then $G$ is
  \indexref{def:abeliangroup} 
...
\begin{remark}[Not every Abelian group is cyclic]
  ...
  \indexref{def:abeliangroup} but not \indexref{def:cyclicgroup}  
\end{remark}
...
\begin{definition}[Direct sum]
  ...
  \cite{wiki:directsum}
  See also \indexref{def:directproduct}.
  
  \label{def:directsum}
\end{definition}

\begin{definition}[Finitely generated abelian group]
  An \indexref{def:abeliangroup} $(G, +)$ is called finitely generated
  \cite{wiki:fgagroup} 
...
\begin{theorem}[The fundamental theorem of finitely generated abelian
    groups]
  Every \indexref{def:fgagroup} $G$ is isomorphic to a
  \indexref{def:directsum} of primary cyclic groups and infinite cyclic
...
\begin{example}[Permutation]
  ...
  As result we have a \indexref{def:group} of permutations.
...
\begin{example}[$S_3$ group]
  ...
  we will have \indexref{def:alternatinggroup} $A_3$.
  \label{ex:s3group}
\end{example}

\begin{example}[$S_3/A_3$ quotient group]
  Lets consider the following \indexref{def:quotientgroup}
...
\begin{example}[Transposition product]
  ...
  The example shows a product of 2 \indexref{def:transposition}s with a
  same element:
  \begin{eqnarray}
    (a,c)(a,b) = 
    \begin{array}{c}
...
\begin{corollary}
  $S_n$ is generated by any combination of a \indexref{def:transposition} and
...
\begin{definition}[Ring]
  ...
  the elements of $R$ forms a \indexref{def:monoid} under 
...
The \indexref{def:ring} is a generalization of integer numbers conception.
\begin{example}[Ring of integers $\mathbb{Z}$]
  The set of integer numbers $\mathbb{Z}$ forms a \indexref{def:ring}
  under $+$ and $\cdot$ operations i.e. addition $\oplus$ is
  $+$ and multiplication $\odot$ is $\cdot$. Thus for integer
  numbers we have the following \indexref{def:ring}:
...
\begin{example}[Multiplicative group of integers modulo $n$]
  ...
  \indexref{def:propersubgroup} not equal to $id$.
...
\begin{example}[Ideal $2 \mathbb{Z}$]
  Consider even numbers. They forms an \indexref{def:ideal} in
...
\begin{example}[Ring of integers modulo $n$: $\mathbb{Z}/n\mathbb{Z}$]
  ...
  The \indexref{def:ring} where the objects are defined is called as
  $\mathbb{Z}/n\mathbb{Z}$.
  \label{ex:intmodulo}
\end{example}

\begin{definition}[Ideal generated by a set]
  Let $R$ be a \indexref{def:ring} and $S$ is a sub set of $R$.
...
domain is a nonzero commutative \indexref{def:ring} in which the
...
\begin{definition}[Principal ideal domain]
  ...
  \indexref{def:integraldomain} in which every ideal is principal, i.e., can be
...
\begin{definition}[Maximal ideal]
  ...
  inclusion) amongst all \indexref{def:properideal}s i.e.
...
\begin{example}[Maximal ideal]
  If $F$ is a \indexref{def:field} then the only maximal ideal is
...
\begin{lemma}[$n\mathbb{Z}$ prime ideal]
  ...
  \indexref{def:primeideal} in $\mathbb{Z}$ \cite{wiki:primeideal}
  \label{lem:primeideal_in_Z}
\end{lemma}

\begin{lemma}
  All nonzero \indexref{def:primeideal}s are maximal in a principal
...
\begin{definition}[Quotient ring]
  ...
  See also \indexref{def:quotientgroup}
  \label{def:quotientring}
\end{definition}

\subsection{Polynomial ring $K\left[X\right]$}

Let we have a commutative \indexref{def:ring} $K$. Lets create a new
\indexref{def:ring} $B$ with the following infinite sets as elements:
\begin{equation}
...
The sequences (\ref{eq:polynomial}) forms a \indexref{def:ring} with
...
original \indexref{def:ring} $K$. Thus $K$ is sub-ring of the new ring
$B$.
...
\begin{definition}[Polynomial ring]
  The \indexref{def:ring} of sequences (\ref{eq:polynomial}) with
...
\begin{theorem}[About irreducible polynomials]
  Let $\pi(X)$ is an \indexref{def:irreducible} in $K\left[X\right]$ and let
  ...
  \begin{proof}
    ...
    \indexref{lem:bezout} we can get $Q,R \in K\left[X\right]$ such
    that
    \[
    \pi(X) R(X) + h(X) Q(X) = 1,
    \]
...
\begin{theorem}[About ideal generated by irreducible polynomial]
  ...
  an \indexref{def:ideal} generated by the polynomial. The $I$ is
  \indexref{def:maxideal} if and only if $P$ is irreducible in
  $K\left[X\right]$
  \begin{proof}
...
\begin{definition}[Field]
  ...
  $\left(R \setminus \{0\}, \odot\right)$ is an \indexref{def:abeliangroup}.
...
\begin{definition}[Unique factorization domain]
   ...
   an \indexref{def:integraldomain}, and in which every non-zero non-unit element
...
\begin{theorem}[About Quotient Ring and Maximal Ideal]
  Let $\left(R, +, \cdot\right)$ is a commutative \indexref{def:ring}
  with additive identity $0_R$ and multiplicative identity $1_R$. Let
  $I$ be an \indexref{def:ideal} of $R$ then $I$ is
  \indexref{def:maxideal} if and only if \indexref{def:quotientring}
  $R/I$ is a \indexref{def:field}
  \label{thm:maxideal}
  \begin{proof}
...
\begin{theorem}[Dedekind]
  If we have $n$ distinct \indexref{def:character}s $\phi_1, \dots,
...
\begin{definition}[Module]
  Let $R$ is a \indexref{def:ring} and $1_R$ is it's multiplicative
  identity. A left R-module $M$ consists of an \indexref{def:abeliangroup}
...
\begin{example}[Module]
  If $K$ is a \indexref{def:field} then concepts of
  K-\indexref{def:vectorspace} and $K$-module are the same
\end{example}
...
\begin{definition}[Free module]
  The \indexref{def:module} that has a basis (i.e. linearly independent
  generating set) 
...
\begin{example}[Direct sum of modules]
  If we have 2 \indexref{def:freemodule}s $M$ and $N$ with bases $m_1,
  m_2, \dots, m_m$ and $n_1, n_2, \dots, n_n$. Then the
  \indexref{def:directsummodules} $A = M \oplus N$ will also be a free
...
\begin{definition}[Cyclic module]
  A (left) $R$-\indexref{def:module} $M$ is called cyclic if $M$ can be
  generated by 
...
\begin{claim}
  ...
  \begin{proof}
    ...
    the claim as a definition of the \indexref{def:cyclicmodule}.
  \end{proof}
...
\begin{definition}[Vector space]
  Let $F$ is a \indexref{def:field}. The set $V$ is called as vector
...
\begin{lemma}[About vector space isomorphism]
  ...
  there exists an \indexref{def:isomorphism} between them
...
\begin{definition}[Rank]
  ...
  \indexref{def:image}: $rg f = \dim \Ima f$ \cite{wiki:rank}:
  \label{def:rank}
\end{definition}

\begin{definition}[Nullity]
  The nullity of a linear map $f: V \to W$ is dimension of
  \indexref{def:kernel}: $nul f = \dim \ker f$ \cite{wiki:kernel}:
  \label{def:nullity}
\end{definition}

\begin{theorem}[Rank–nullity theorem]
  Let $V$ and $W$ be vector spaces over some field and let $T: V \to
  W$ be a linear map. Then the \indexref{def:rank} of $T$ is the
  dimension of the image of $T$ and the \indexref{def:nullity} of $T$
...
\begin{definition}[General linear group of a vector space]
  If $V$ is a \indexref{def:vectorspace} over field $K$  the general
...
\begin{definition}[Dual basis]
  ...
  \indexref{def:dualspace} such that $v_j(e_i) = \delta_{ij}$ and is
...
\begin{definition}[Non-degenerate bilinear form]
  ...
  meaning that the map from $V$ to $V^\ast$ (the \indexref{def:dualspace}
...
\begin{definition}[Bijection]
  ...
  correspondence) if it is an \indexref{def:injection} and a
  \indexref{def:surjection}. 
  \label{def:bijection}
\end{definition}
...
\begin{definition}[Isomorphism]
  If a map is \indexref{def:bijection} as well as
  \indexref{def:homomorphism} when it is called as isomorphism.
...
\begin{theorem}[First isomorphism]
  ...
  surjective \indexref{def:homomorphism}. Then if $N = \ker \phi$ we
  have
  \[
  H \cong G/N
  \]
...
\begin{theorem}[Isomorphism extension theorem]
  Let $F$ is a \indexref{def:field} and $E$ is an
  \indexref{def:algebraicextension} of $F$.
  $F'$ is another \indexref{def:field} and $E'$ the 
  \indexref{def:algebraicextension} of $F'$.

  If there exists an \indexref{def:isomorphism} $\phi: F \to F'$ then
  it can be extended to an isomorphism $\tau: E \to E'$.

  \begin{proof}
    The proof of the isomorphism extension theorem depends on
    \indexref{lem:zorn}'s lemma.
...
\begin{theorem}[About group homomorphism]
  A group homomorphism is \indexref{def:injection} iff the kernel is
