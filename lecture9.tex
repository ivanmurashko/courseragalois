...
\indexref{def:splittingfield} of $P$. We have already done the work for
several types of polynomials: \indexref{def:cyclotomicpolynomial}s,
\indexref{sec:kummerextension} and so on.
...
\indexref{def:galoisgroup}. If $K$ is the splitting field of the
...
\footnote{
  ...
  \sqrt[5]{2}\right)$ are \indexref{def:galoisextension}s.
  By \indexref{thm:galoiscorrespondence} one can get that
  \[
...
\begin{definition}[Integral element]
  Let $A$ be an \indexref{def:integraldomain}, i.e. a ring without zero
  divisors and let $B$ is an extension of $A$. The element $\alpha
  \in B$ is called integral over $A$ if $\alpha$ is a root of a
  \indexref{def:monicpolynomial} $P \in A\left[X\right]$.
...
\begin{example}
  ...
  $\sqrt{2}$ is an \indexref{def:integralelement} over $\mathbb{Z}$.
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      $A$-\indexref{def:cyclicmodule} i.e. module with one generator.
    ...
    $\alpha$ as a root.
    \footnote{
      The polynomial will be a \indexref{def:monicpolynomial} because
...
\begin{definition}[Integral extension]
  ...
  B$, $\alpha$ is an \indexref{def:integralelement} over $A$.  
...
\begin{proposition}
  ...
  over $B$ then $C$ is an \indexref{def:integralextension} over $A$.
  \begin{proof}
...
\begin{proposition}
  ...
  an \indexref{def:integralelement} over $A$.
  \begin{proof}
    Proof is left as an exercise
    \footnote{
...
\begin{definition}[Integrally closed]
  ...
  integrally closed in \indexref{def:fractionfield} $\mathrm {Frac}(A)$.
  \label{def:integrallyclosed}
\end{definition}

\begin{example}
  $\mathbb{Z}$ is \indexref{def:integrallyclosed}.
  \footnote{
...
\begin{remark}
  More generally any
  \indexref{def:ufd} is \indexref{def:integrallyclosed}.
  \begin{proof}
    Let $A$ be a \indexref{def:ufd} and $x \in \mathrm {Frac}(A)$ such
    that $x \ne 0$. So
...
Let $K$ is a \indexref{def:numberfield} and $\left[K:\mathbb{Q}\right]
= N$.
\begin{definition}[Ring of integers]
  Let $K$ is a \indexref{def:numberfield}. The ring of integers
...
\begin{property}
  ...
  \begin{proof}
    ...
    root of some \indexref{def:monicpolynomial}
    $Q \in \mathbb{Z}\left[X\right]$.
    Then the $P_{min} \mid Q$. So $Q = P_{min} R$.
    If we pick $P_{min}$ to be monic, then by an
    argument very similar to that of the \indexref{lem:gauss} lemma,
...
\begin{remark}[$K$ embedding of $E$ into the algebraic closure
  ...
  \indexref{def:homomorphism} of \indexref{def:kalgebra}s i.e. the map
...
the extension $E$ is \indexref{def:separableextension}. If the
...
\footnote{
  ...
  \nonumber
  \end{eqnarray}
  where $\left[E:K\right]_i$ is \indexref{def:inseparabledegree}.
}
but for simplicity,
...
\begin{remark}
  ...
  \begin{proof}
    ...
    \indexref{def:norm} does not depend on the order and the
    result become the same after the permutation.

    The same result is for \indexref{def:trace}.
  \end{proof}
\end{remark}
...
\begin{property}
  ...
  \item If we have the tower of extensions $K \subset F \subset E$
    ...
    \footnote{      
      ...
      \ref{rem:lec9_add}).
      The same is valid for \indexref{def:norm}.  
    }
  ...
  \begin{proof}
    ...
    K\right)$. The \indexref{def:norm} is a product and it's assigned
    ...
    \footnote{
      ...
      \indexref{def:trace} is a coefficient of the polynomial i.e.
      \(
      \mathrm{Tr}_{\mathbb{Q}(\alpha)/\mathbb{Q}}\left(
      \alpha
      \right) \in \mathbb{Z}
...
\footnote{
  ...
  \end{cases}
  \]
  Therefore the \indexref{def:endomorphism} matrix can be written as
  follows: 
  \[
  M = 
  \begin{bmatrix}
...
\indexref{def:endomorphism} of a vector space $E/K$, and
...
\begin{theorem}
  ...
  $\mathbb{Z}$-module that is a \indexref{def:freemodule} of
  ...
  \begin{proof}
    ...
    \indexref{def:fgagroup} and we know a lot of things about such
    groups. The \indexref{def:fgagroup} is the same as
    ...
    \]
    this is
    \indexref{def:nondegeneratebf} (see 4th property
    \ref{property:lec9_norm_trace}) therefore $\exists v_1, \dots,
    v_n$ - \indexref{def:dualbasis} ($\mathbb{Q}$-basis of $K$)
    ...
    \footnote{
      ...
      \indexref{def:surjection} by the $f_x$ construction. The map is
      also \indexref{def:injection} as soon as the map $T: K \times K \to \mathbb{Q}$ is
      \indexref{def:nondegeneratebf} and as result the $f_x(y) =
      T(x,y)$ has a trivial kernel (if $x \ne 0$): $\ker f =
      \{0\}$. Therefore we can conclude that $K^\ast$ is
      \indexref{def:dualspace}. I.e. exists a set of elements 
      $\{f_{v_j}\} \subset K^\ast$  which
      form the \indexref{def:dualbasis} to $\{e_i\}$ i.e. $f_{v_j}(e_i) =
...
coefficients. $K$ is a \indexref{def:splittingfield} of $P$ over
...
\footnote{
  Let $J$ is an \indexref{def:ideal} of $A$, then $A J = J$
...
\footnote{
  ...
  \indexref{def:maxideal} of $A$ then by remark
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \indexref{def:transitive}. By \indexref{thm:chineseremainder} theorem
    ...
    \footnote{
      We have $J_i$ is a \indexref{def:maxideal} and $A$ is a
      \indexref{def:pid}. Therefore by lemma \ref{lem:primeideal_is_maximal},
      $J_i$ is a \indexref{def:primeideal}
      that contains $p  
    ...
    $J_1$ is a \indexref{def:primeideal} (see note
    ...
    \footnote{
      i.e. that there is a
      \indexref{def:surjection} or in other words that $\forall \bar{g}_i
      \in Gal\left(k_i/\mathbb{F}_p\right), \exists d_i \in D_i$ such that
      $d_i \to \bar{g}_i$.
    }
    We may assume that $i=1$. By the
    \indexref{thm:primitiveelement} theorem $\exists z \in
    \mathbb{F}_p$ such that $k_1 = \mathbb{F}_p\left(z\right)$
    i.e. $z$ generates $k_i/\mathbb{F}_p$.
    By \indexref{thm:chineseremainder} theorem $\exists y \in A$ such that
    ...
    moreover the roots
    are \indexref{def:integralelement}s over $\mathbb{Z}$
    \footnote{
