...
\begin{lemma}
  ...
  \begin{proof}
    ...
    \footnote{
      ...
      $A$-\nameref{def:cyclicmodule} i.e. module with one generator.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    v_n$ - \nameref{def:dualbasis} ($\mathbb{Q}$-basis of $K$)
    ...
    \footnote{
      ...
      \nameref{def:surjection} by the $f_x$ construction. The map is
      also \nameref{def:injection} as soon as the map $T: K \times K \to \mathbb{Q}$ is
      \nameref{def:nondegeneratebf} and as result the $f_x(y) =
      T(x,y)$ has a trivial kernel (if $x \ne 0$): $\ker f =
      \{0\}$. Therefore we can conclude that $K^\ast$ is
      \nameref{def:dualspace}. I.e. exists a set of elements 
      $\{f_{v_j}\} \subset K^\ast$  which
      form the \nameref{def:dualbasis} to $\{e_i\}$ i.e. $f_{v_j}(e_i) =
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \footnote{
      i.e. that there is a
      \nameref{def:surjection} or in other words that $\forall \bar{g}_i
    ...
    moreover the roots
    are \nameref{def:integralelement}s over $\mathbb{Z}$
    \footnote{
