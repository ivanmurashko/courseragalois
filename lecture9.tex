...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \footnote{
      As soon as the map $T: K \times K \to \mathbb{Q}$ is
      \nameref{def:nondegeneratebf} we can define a linear map $f_x(y) =
      T(x,y)$ the map has a trivial kernel (if $x \ne 0$): $\ker f =
      \{0\}$. I.e. for any $x \in K$ we have a map $f_x: K \to
      \mathbb{Q}$.
      The map is between $K$ and its \nameref{def:dualspace} $K^\ast$
      (??? prove that $x \to f_x$ is isomorphism). I.e. exists a set of elements
      $\{f_{v_j}\} \subset K^\ast$  which
      form the \nameref{def:dualspace} basis to $\{e_i\}$ i.e. $f_{v_j}(e_i) =
